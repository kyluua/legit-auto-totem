...
public final class InventoryHelper {
	...
	public static boolean canClickInventory(Minecraft minecraft) {
		...
		return minecraft.screen == null || minecraft.screen instanceof InventoryScreen;
	}
	...
	public static void closeOwnInventory(Minecraft minecraft, Screen opened) {
		...
		if (player == null || opened == null || minecraft.screen != opened) {
			return;
		}
	...
	private static void click(Minecraft minecraft, int networkSlot, int button, ClickType input) {
		LocalPlayer player = minecraft.player;
		...
		minecraft.gameMode.handleInventoryMouseClick(player.inventoryMenu.containerId,
	...
	public static boolean moveToHotbarSlot(Minecraft minecraft, int sourceIndex, int hotbarIndex,
		...
		if (method == SwapMethod.SWAP) {
			...
			click(minecraft, toNetworkSlot(sourceIndex), hotbarIndex, ClickType.SWAP);
			return true;
		}

		click(minecraft, toNetworkSlot(sourceIndex), 0, ClickType.PICKUP);
		click(minecraft, toNetworkSlot(hotbarIndex), 0, ClickType.PICKUP);
	...
	public static boolean moveToOffhand(Minecraft minecraft, int sourceIndex, SwapMethod method) {
		...
		if (method == SwapMethod.SWAP) {
			...
			click(minecraft, toNetworkSlot(sourceIndex), OFFHAND_SWAP_BUTTON, ClickType.SWAP);
			return true;
		}

		click(minecraft, toNetworkSlot(sourceIndex), 0, ClickType.PICKUP);
		click(minecraft, OFFHAND_NETWORK_SLOT, 0, ClickType.PICKUP);
